...
void testApp::setup(){
	...
	ofSetWindowTitle("mouse gesture classification example");
...
void testApp::update(){
    ...
    if(recording)
        ...
        recordings.back().push_back(ofMousePosition(mouseX, mouseY));
    frame++;
}
...
void testApp::draw(){
    ...
    if(!recordings.empty())
        for(vector<ofMousePosition>::iterator movement=recordings.back().begin(); movement!=recordings.back().end(); movement++)
        {
            double color_ratio = double(movement-recordings.back().begin()) / recordings.back().size();
            ofSetColor(255*color_ratio,0,255*(1-color_ratio),255);
            
            ofRect(movement->x,movement->y,5,5);
        }
}
...
void testApp::mousePressed(int x, int y, int button)
{
    ...
    recordings.push_back(vector<ofMousePosition>());
    recording = true;
}
